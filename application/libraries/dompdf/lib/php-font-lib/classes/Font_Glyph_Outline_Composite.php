...
class Font_Glyph_Outline_Composite extends Font_Glyph_Outline
{
    ...
    function parseData()
    {
        ...
        do
        {
            ...
            else if ($flags & self::WE_HAVE_AN_X_AND_Y_SCALE)
            {
                $a = $font->readInt16();
                $d = $font->readInt16();
            }
            else if ($flags & self::WE_HAVE_A_TWO_BY_TWO)
            {
                $a = $font->readInt16();
                $b = $font->readInt16();
                $c = $font->readInt16();
    ...
    function encode()
    {
        ...
        foreach ($this->components as $_i => $_component)
        {
            ...
            else if ($_component->point_component > 0xFF || $_component->point_compound > 0xFF)
            {
                $flags |= self::ARG_1_AND_2_ARE_WORDS;
            }
