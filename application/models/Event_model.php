...
class Event_model extends My_Model
{
    ...
    public function list_market_types($dataArray = array())
    {
        ...
        if (isset($dataArray['status'])) {
            $this->db->where('mbo.status', $dataArray['status']);
        }

        $this->db->order_by('mt.market_start_time', 'DESC');
    ...
    public function get_all_fancy($dataArray = array())
    {
        $this->db->select('*');
        $this->db->from('market_book_odds_fancy');
        ...
        if (!empty($dataArray)) {
            ...
            if (isset($dataArray['cron_disable'])) {
                $this->db->where('cron_disable', $dataArray['cron_disable']);
            }
        }
        $return = $this->db->get()->result_array();
        return $return;
    ...
    public function get_casino_events()
    {
        $this->db->select('*');
        $this->db->from('list_events');
        $this->db->where('is_casino', 'Yes');
    ...
    public function delete_market_book_odds($dataValues)
    {
        $this->db->where('event_id', $dataValues['event_id']);
        $this->db->where('created_at <= ', $dataValues['datetime']);
        $return = $this->db->delete('market_book_odds');

        return $return;
    }

    public function delete_market_types($dataValues)
    {
        $this->db->where('event_id', $dataValues['event_id']);
        $this->db->where('created_at <= ', $dataValues['datetime']);
        $return = $this->db->delete('market_types');

        return $return;
    }

    public function delete_market_book_odds_runners($dataValues)
    {
        $this->db->where('event_id', $dataValues['event_id']);
        $this->db->where('created_at <= ', $dataValues['datetime']);
        $return = $this->db->delete('market_book_odds_runner');
    ...
    public function list_events_by_event_type($dataArray = array())
    {
        $this->db->select('le.competition_id,le.event_type,le.event_id,le.event_name,le.open_date,le.score_match_id,le.is_score');
        $this->db->from('list_events as le');
        if (isset($dataArray['event_type'])) {
            $this->db->where('le.event_type', $dataArray['event_type']);
        }
    ...
    public function toggle_score($dataValues)
    {
