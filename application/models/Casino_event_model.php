...
class Casino_event_model extends My_Model
{
    ...
    public function list_event($dataArray = array())
    {
        $this->db->select('le.competition_id,le.event_type,le.event_id,le.event_name,le.open_date');
        $this->db->from('list_events as le');
        if (isset($dataArray['event_short_name'])) {
            $this->db->where('le.event_short_name', $dataArray['event_short_name']);
        }
    ...
    {
        $this->db->select('mt.market_id,mt.round_id,mt.market_name,mt.market_start_time,mt.runner_1_selection_id,mt.runner_1_runner_name,mt.runner_2_selection_id,mt.runner_2_runner_name,mbo.status,mbo.inplay,mbo.complete,mt.timer');
        ...
        $this->db->where('mbo.status', 'OPEN');
        $this->db->or_where('mbo.status', 'ACTIVE');
        $this->db->group_end();
