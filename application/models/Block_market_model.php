...
// p($this->db->last_query());
    ...
    public function getSingleBlockMarketsByUserId($dataValues)
    {
        $this->db->select('*');
        $this->db->from('block_markets');
        $this->db->where($dataValues);
    ...
    public function checkFancyBlockMarketAllowExists($dataValues)
    {
        $this->db->select('*');
        $this->db->from('fancy_block_markets_allow');
        $this->db->where($dataValues);
        $return = $this->db->get()->row();
        return $return;
    }

    public function addFancyBlockMarketAllow($dataValues)
    {
        ...
        if (count($dataValues) > 0) {
            if (array_key_exists('block_market_id', $dataValues) && !empty($dataValues['block_market_id'])) {
                ...
                $this->db->where('block_market_id', $dataValues['block_market_id']);
                $this->db->delete('fancy_block_markets_allow');
                $block_market_id = $dataValues['block_market_id'];
            } else {
                $dataValues["created_at"] = date("Y-m-d H:i:s");
                $this->db->insert('fancy_block_markets_allow', $dataValues);
                $block_market_id = $this->db->insert_id();
            }
        }
    ...
    public function getFancyBlockMarketsAllowedByUserId($dataValues)
    {
        $this->db->select('*');
        $this->db->from('block_markets');
        $this->db->where($dataValues);
