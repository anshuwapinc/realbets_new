    ...
    foreach ($events as $event) {
        ...
        if (!empty($event['market_types'])) {
            foreach ($event['market_types'] as $market_type) {
                ...
                if($event['event_type'] == 2 || $event['event_type'] == 1)
                {
...
?>
                    ...
                    <div class="market-listing-table 1190470587  matchTable1190470587" id="matchTable31057636">
                        ...
                        <div class="game-head">
                            <ul class="match-btn">
                                ...
                                <li>
                                    <a class="<?php echo get_user_type() != 'User'?'btn-pin':'btn-pin-user' ?> 31057636"><?php echo $market_type['market_name']; ?></a>
                                </li>

                                <?php

                                if (get_user_type() != 'User') { ?>
                                    <li> <a class="btn-refresh" onclick="fetchMatchOddsPositionList('<?php echo $event['event_id']; ?>' ,'<?php echo $market_type['market_id']; ?>')"><i class="fas fa-boxes"></i> Position</a>
                                    </li>
                                <?php }
                        ...
                        <?php
                                } else if ($market_type['market_name'] == 'Toss') { ?>
                            Min :<?php echo $market_type['user_info']->min_stake; ?>Max :50000 <?php echo $market_type['bookmaker_user_info']->bet_delay; ?> sec
                        </div>
                ...
                <?php } else { ?>
                    ...
                    Min :<?php echo $market_type['user_info']->min_stake; ?>Max :<?php echo $market_type['user_info']->max_stake; ?> <?php echo $market_type['bookmaker_user_info']->bet_delay; ?> sec
                </div>
            <?php } ?>
            ...
            <div>
                ...
                ?>
                        ...
                        <div id="user_row " class="odds_rows runner-row-10301 matchOpenBox_<?php echo str_replace('.', '', $runner['market_id']) ?>_<?php echo $runner['selection_id'] ?>">
                            <div class="events_odds">
                                ...
                                <div class="valuename">
                                    ...
                                    ?>
                                    <input type="hidden" class="position_<?php echo  str_replace('.', '', $runner['market_id']); ?> loss_profit_<?php echo  str_replace('.', '', $runner['market_id']).'_'.$runner['selection_id']; ?>" id="selection_<?php echo $runner['sort_priority']; ?>" data-id="<?php echo isset($runner['selection_id']) ? $runner['selection_id'] : ''; ?>" value="<?php echo $exposure; ?>">
                                </div>
                            </div>
