    ...
    <div class="container-fluid listing-grid">
        <div class="detail-row">
            <div class="heading-container">
                <h2 class="heading">Withdraw Requests</h2>
            ...
            ?>
            <?php
            if (!empty($this->session->flashdata('operation_error_msg'))) {
            ?>
             <div class="row">
                    <div class="col-md-12">
                <div class="alert alert-danger">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <?php echo $this->session->flashdata('operation_error_msg'); ?>
                </div>
                    </div>
             </div>
            <?php
                    ...
                    <tbody id="filterdata">
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td><?php echo !empty($deposit_request['bank_name']) ? $deposit_request['bank_name'] : '-'; ?></td>
                                    <td><?php echo !empty($deposit_request['account_no']) ? $deposit_request['account_no'] : '-'; ?></td>
                                    <td><?php echo !empty($deposit_request['ifsc_code']) ? $deposit_request['ifsc_code'] : '-'; ?></td>
                                    <td><?php echo !empty($deposit_request['account_holder_name']) ? $deposit_request['account_holder_name'] : '-'; ?></td>
                                    ...
                                    <td class="d-flex action_icon">
                                        ...
                                        <!-- <a class="btn btn-success" onclick="return confirm('Are you sure you want to confirm the Withdraw request')"  href="confirm-withdraw/<?php echo $deposit_request['id'] ?>/<?php echo $deposit_request['user_id'] ?>/<?php echo abs($deposit_request['amount']) ?>" title="Success">Confirm</a> -->
                                        <a  class="btn btn-success btn-xs" href="change-withdraw-request-status/Confirm/<?php echo $deposit_request['id'] ?>" id="remarks_link<?php echo $i ?>" title="Confirm" style="margin-right:5px">Confirm</a>
                                    
                                     <a  class="btn btn-danger btn-xs" href="change-withdraw-request-status/Reject/<?php echo $deposit_request['id'] ?>" id="remarks_link<?php echo $i ?>_reject" title="Reject">Reject</a>
                                        <!-- <a class="btn btn-danger" href="change-withdraw-request-status/Reject/<?php echo $deposit_request['id'] ?>" onclick='return validate_reject_btn("remarks_<?php echo $i ?>")' id="remarks_link<?php echo $i ?>" title="Reject">Reject</a> -->
                                    </td>
   ...
   $(document).ready(function() {
    ...
    function addremark(val, id) {
        ...
        if (val.trim() != "") {
            ...
            var newlink1 = $(`#${id}`+'_reject').attr("href") +"/"+ val.replaceAll(" ", "-");
            $(`#${id}`).attr("href",newlink);
            $(`#${id}`+'_reject').attr("href",newlink1);
        }
    }
